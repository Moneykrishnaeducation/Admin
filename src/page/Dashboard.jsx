...
const Dashboard = () => {
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="flex-1 w-full text-yellow-400 px-3 sm:px-6 lg:px-8 py-4 transition-all duration-300">

        {/* Stats Buttons */}
        <div className="flex flex-col sm:flex-row flex-wrap justify-center sm:justify-between gap-3 sm:gap-4 mb-6">
          <button
            className={`flex items-center justify-center gap-2 px-4 py-2 sm:px-6 sm:py-3 rounded-md transition-all text-sm sm:text-base w-full sm:w-auto ${isDarkMode
                ? "bg-yellow-500 text-black hover:bg-yellow-300"
                : "bg-yellow-600 text-white hover:bg-yellow-700"
              }`}
            onClick={() => downloadCSV("/api/export/users/csv/", "users.csv")}
          >
            <Download className="w-4 h-4 sm:w-5 sm:h-5" /> User Account
          </button>
          ...
          >
            <Download className="w-4 h-4 sm:w-5 sm:h-5" /> Trading Account
          </button>
          ...
          >
            <Download className="w-4 h-4 sm:w-5 sm:h-5" /> Transaction
          </button>
        </div>
          ...
          {stats.map((stat, index) => (
            <Link key={index} to={stat.to}>
              ...
              >
                ...
                <h2
                  className={`text-base sm:text-lg font-semibold mt-1 ${stat.value < 10
                      ? "text-gray-600"
                      : stat.value < 100
                        ? "text-orange-500"
                        : "text-green-600"
        ...
        <div className="mt-6 sm:mt-10">
          <h2
            className={`text-lg sm:text-2xl font-bold mb-3 sm:mb-4 ${isDarkMode ? "text-white" : "text-gray-900"
              }`}
          >
            Recent Activities
          </h2>
